...
void callback(const sensor_msgs::ImageConstPtr& original_image)
{
    ...
    std::vector<int16_t> grass_pos;
    std_msgs::Int16MultiArray grass_pos_msg;
    ...
    {
      curr_grass = boundRect[i];
      // Filter out extremely tiny objects
      if(curr_grass.height > size_threshold && curr_grass.width > size_threshold)
      {
	// Draw rectangles on image around grasss
	cv::rectangle(cv_ptr->image, curr_grass.tl(), curr_grass.br(), \
		    color, 1, 8, 0 );
	// Push grass attributes to vector of all grass attributes
	grass_pos.push_back(curr_grass.x);
	grass_pos.push_back(curr_grass.y);
	grass_pos.push_back(curr_grass.width);
	grass_pos.push_back(curr_grass.height);
	// Label grasss with text
	cv::putText(cv_ptr->image, "GRASS", curr_grass.tl(), \
    ...
    grass_pos_msg.data = grass_pos;
    pub.publish(grass_pos_msg);
}

int main(int argc, char **argv)
{
    ros::init(argc, argv, "grass_detection");

    ros::NodeHandle nh;

    pub = nh.advertise<std_msgs::Int16MultiArray>("grass_positions", 1000);
